...
from datasets import CoNLL2003Dataset, collate_fn_char
from models.BiLSTM_CharCNN_TransformerEncoder import BiLSTM_CharCNN_TransformerEncoder
...
train_loader = DataLoader(
    ...
    shuffle=True,
    collate_fn=lambda x: collate_fn_char(x, char2idx)
)
...
model = BiLSTM_CharCNN_TransformerEncoder(
    ...
    linear_output_dim, label_size, embedding_mat
)
...
for epoch in range(epochs):
    ...
    for data, target, words, char_data in train_loop:
        ...
        optimizer.zero_grad()
        pred = model(data, case_data, char_data, seq_lengths)
    ...
    if best_f1 < val_f1:
        ...
        torch.save(checkpoint, 'blstm6.pt')
    ...
    if best_loss > val_loss:
        ...
        torch.save(checkpoint, 'blstm6_loss.pt')
        print('update best loss from {:.4f} to {:.4f}'.format(best_loss, val_loss))
        best_loss = val_loss

predict_model = BiLSTM_CharCNN_TransformerEncoder(
    vocab_size, char_size, embedding_dim, char_embedding_dim, 
    char_final_dim, case_final_dim,
    lstm_hidden_dim, lstm_num_layers, lstm_dropout, 
    transformer_hidden_dim, transformer_num_layers, transformer_dropout,
...
checkpoint = torch.load('blstm6.pt')
...
with open('dev6.out', 'w') as f:
    with torch.no_grad():
        ...
        for data, targets, words, char_data in tqdm(val_loader):
            ...
            preds = predict_model(data, case_data, char_data, seq_lengths)
