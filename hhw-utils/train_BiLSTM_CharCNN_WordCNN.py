...
from models.BiLSTM_CharCNN_WordCNN import BiLSTM_CharCNN_WordCNN
...
for epoch in range(epochs):
    ...
    if best_f1 < val_f1:
        ...
        torch.save(checkpoint, 'blstm5.pt')
    ...
    if best_loss > val_loss:
        ...
        torch.save(checkpoint, 'blstm5_loss.pt')
        print('update best loss from {:.4f} to {:.4f}'.format(best_loss, val_loss))
        best_loss = val_loss

predict_model = BiLSTM_CharCNN_WordCNN(
    vocab_size, char_size, embedding_dim, char_embedding_dim, 
    char_final_dim, word_final_dim, case_final_dim,
    lstm_hidden_dim, lstm_num_layers, lstm_dropout, 
    linear_output_dim, label_size, embedding_mat
)
checkpoint = torch.load('blstm5.pt')
